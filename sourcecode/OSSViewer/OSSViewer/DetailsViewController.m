...
@property (strong, nonatomic) UIPopoverController *photoPopoverController;
@end

@implementation DetailsViewController
@synthesize photoPopoverController = _photoPopoverController;
...
-(void) refresh
{
    ...
    self.navigationItem.title = [NSString stringWithFormat:@"%@'Objects",self.bucketName];
...
- (void)viewDidLoad
{
    ...
    _addItem= [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAdd  target:self action:@selector(addOSSObject:)];
...
-(void) addOSSObject:(id) sender
{
     ...
     */
    if (self.bucketName == nil || [self.bucketName isEqualToString:@""]) {
        return;
    }
  
     AddObjectViewController *addVC = [[AddObjectViewController alloc] initWithNibName:@"AddObjectViewController" bundle:nil];
    addVC.modalPresentationStyle = UIModalPresentationFormSheet;
    addVC.delegate = self;
    [self presentModalViewController:addVC animated:YES];
    [addVC release];
    /*
    if (_addPopoverController == nil) {
        AddObjectViewController *addVC = [[AddObjectViewController alloc] initWithNibName:@"AddObjectViewController" bundle:nil];
        addVC.delegate = self;
        _addPopoverController = [[UIPopoverController alloc] initWithContentViewController:addVC];
        _addPopoverController.delegate = self;
        [addVC release];
    }
    if (_addPopoverController.isPopoverVisible) {
        [_addPopoverController dismissPopoverAnimated:YES];
    }
    else
    {
        [_addPopoverController presentPopoverFromBarButtonItem:sender
                                        permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
    }
     */
     
}
-(void)onYellowLightTimer:(int) index
{
    switch (_index) {
        case 0:
        {
...
-(void) AddObjectSelectedIndex:(int) index
{
    ...
    [NSTimer scheduledTimerWithTimeInterval:(0.3) target:self selector:@selector(onYellowLightTimer:) userInfo:nil repeats:NO];
}
...
-(void) addPictureFromCamera
{
    ...
    if ([UIImagePickerController isSourceTypeAvailable:
        ...
        if (_photoPopoverController == nil) {
            _photoPopoverController = [[UIPopoverController alloc] initWithContentViewController:imagePicker];
        }
       
        [_photoPopoverController presentPopoverFromBarButtonItem:_addItem permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
        [imagePicker release];
    } else {
...
-(void)addPictureFromPhotoLibrary
{
    ...
    picker.allowsEditing = NO;
    if (_photoPopoverController == nil) {
        _photoPopoverController = [[UIPopoverController alloc] initWithContentViewController:picker];
    }
   [_photoPopoverController presentPopoverFromBarButtonItem:_addItem permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES]; //   [self presentModalViewController:picker animated:YES];
    [picker release];
}
